##################################################
## Description: Create reduced SNP dataset of 
## just AIMs from the enormous dataset of all SNPs
...
## Isolate the AIMs
SNP_list <- intersect(SNP_list, AIMs)
n        <- length(SNP_list)

## Read in list of SNPs in high LD (correlation) to the SNPs we requested
LD_SNPs  <- read.csv('Data/SNP_Proxy.csv', header = TRUE)

## Read in list of SNPs in SNP dataset (huge datasest)
...
## Create matrix to store genotype data into
...
## Combine each subject's 2 alleles into 1 column
...
# Write output
# write.table(clean_data, file = "AIM_SNPs.txt", quote = FALSE, row.names = FALSE)
